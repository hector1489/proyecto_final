...
const getPedidosByUsuario = async (id_usuario) => {
    return await db('SELECT * FROM pedidos WHERE id_usuario = $1;', [id_usuario])
}

const getPedidoDetails = async (id) => {
    return await db('SELECT * FROM pedidos WHERE id = $1;', [id])
}

const updatePedidoStatus = async (id, estado) => {
    return await db('UPDATE pedidos SET estado = $2 WHERE id = $1 RETURNING *;', [id, estado])
}

const cancelPedido = async (id) => {
    return await db('DELETE FROM pedidos WHERE id = $1 RETURNING *;', [id])
}
